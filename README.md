...
Python파일을 안드로이드 환경에서 빌드하는 과정에서 문제가 생겨 최종앱을 빌드하지못했습니다. 시간적문제로 빌드하지못한채, 프로그램을 마무리했습니다.
DB구축이 되어있질않아서 실데이터를 넣어 앱에 적용해보지못했습니다.
꼼꼼한 사전조사, 확실한 계획과 적절한 시간 분배로 다음에는 빌드까지 완성해보는것이 목표입니다.
